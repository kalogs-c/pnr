...
static mpc_result_t* mpc_grow_results(mpc_input_t* i, int j,
                                      mpc_result_t* results_stk,
  ...
  if (j == MPC_PARSE_STACK_MIN) {
    ...
    memcpy(tmp_results, results_stk,
           sizeof(mpc_result_t) * MPC_PARSE_STACK_MIN);
  } else if (j >= MPC_PARSE_STACK_MIN) {
    int results_slots = j + j / 2;
    tmp_results =
        mpc_realloc(i, tmp_results, sizeof(mpc_result_t) * results_slots);
  }
...
static int mpc_parse_run(mpc_input_t* i, mpc_parser_t* p, mpc_result_t* r,
  ...
  case MPC_TYPE_SEPBY1:
    ...
    if (mpc_parse_run(i, p->data.sepby1.x, &results[j], e, depth + 1)) {
      ...
      while (mpc_parse_run(i, p->data.sepby1.sep, &results[j], e, depth + 1) &&
